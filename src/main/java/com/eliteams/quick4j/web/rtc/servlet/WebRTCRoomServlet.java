...
//			String user = UUID.randomUUID().toString().replace("-", "");//生成一个用户ID串
			User userModal = (User)request.getSession().getAttribute("userInfo");
			String user = userModal.getUsername();
//			String user = (String)SessionUtils.getSession().getAttribute("userInfo");
...
//			request.setAttribute("user", user);
			...
			builder.append("<initiator>" + initiator + "</initiator>");
			builder.append("<roomLink>" + roomLink + "</roomLink>");
			builder.append("<roomKey>" + roomKey + "</roomKey>");
...
//			request.getRequestDispatcher("/rtc.jsp").forward(request, response);
		}
	}
}
